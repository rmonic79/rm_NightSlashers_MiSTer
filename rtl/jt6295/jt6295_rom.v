...
always @(posedge clk) begin
    case(st)
        8'b1,8'b10: begin
            rom_addr   <= adpcm_addr;
            // HARDENING (port BW 957b5a7, fragilita' refit): latch SOLO con addr stabile
            // >=2 clk e rom_ok (prima: latch cieco ogni clk). Se il dato non arriva entro
            // la deadline resta il byte PRECEDENTE (sample ripetuto, quasi inudibile)
            // invece di un byte di un flusso sbagliato che fa divergere il predictor
            // ADPCM (glitch). A regime (hit cache) identico a prima.
